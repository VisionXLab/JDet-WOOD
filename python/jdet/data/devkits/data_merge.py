...
def data_merge_result(result_pkl,work_dir,epoch,name,dataset_type,images_dir=""):
    # ``get_classes_by_name`` has always treated DOTA1 as the exact DOTA-v1.0
    # alias, and the Point2RBox-v3 end-to-end config uses that canonical
    # upstream spelling.  Keep the config unchanged and accept the alias at
    # the merge boundary as well.
    supported = [
        "FAIR", "DOTA", "DOTA1", "DOTA1_5", "DOTA2", "FAIR1M_1_5"
    ]
    assert dataset_type in supported, (
        "need to set dataset.test.dataset_type in the config file. "
        "FAIR, FAIR1M_1_5, DOTA/DOTA1, DOTA1_5 and DOTA2 are supported"
    )
    print("Merge results...")
