# Ported from Point2RBox-v3 `mmrotate/models/losses/vis.py`.
# for Point2RBox-v3-jittor. Debug visualization helpers for the Voronoi /
# watershed / SAM-mask pipeline. Function signatures are unchanged.
#
# Framework-tensor inputs (torch.Tensor in the reference) are accepted as
# jt.Var or np.ndarray: every `.detach().cpu().numpy()` / `.permute(...)` site
# was replaced by the `_to_numpy` helper + numpy transposes. Plotting logic is
...
def _to_numpy(x):
    """np.ndarray passthrough; jt.Var (or anything with .numpy()) converted."""
...
def _to_hwc(img):
    """CHW tensor/array -> HWC numpy; HW stays as-is (mirrors ref permute)."""
    arr = _to_numpy(img)
    if arr.ndim == 3:
...
def plot_gaussian_voronoi_watershed(original_image, cls_bg, markers,
    ...
    # Convert tensor to numpy array (CHW -> HWC, or keep HW)
    orig_img = _to_hwc(original_image)

    # Normalize image to [0,1]
    orig_img = (orig_img - orig_img.min()) / (orig_img.max() - orig_img.min())

    cls_bg_np = _to_numpy(cls_bg)
    markers_np = _to_numpy(markers)
...
def visualize_loss_calculation(image, mask_tensor, mu_j, V_j, xy_centered, xy_rotated, max_x, max_y, L_j, L_target, instance_loss, j, class_id):
    ...
    ax = axes[0, 2]
    xy_centered_np = _to_numpy(xy_centered)
    ...
    scatter = ax.scatter(xy_centered_np[:, 0], xy_centered_np[:, 1],  # noqa: F841
    ...
    ax = axes[1, 0]
    xy_rotated_np = _to_numpy(xy_rotated)
    ...
    max_x_np = _to_numpy(max_x)
    max_y_np = _to_numpy(max_y)
    ...
    L_j_np = _to_numpy(L_j)
    L_target_np = _to_numpy(L_target)
    ...
    ax.text(0.5, 0.1, f"GWD Loss: {float(_to_numpy(instance_loss)):.6f}",
...
def save_debug_visualization(image, masks, scores, shape_metrics, metrics_values, best_mask_idx, class_id, metric_name):
    ...
    # Convert image to numpy array for visualization (CHW -> HWC)
    img_np = _to_hwc(image)
