# Ported from Point2RBox-v3 `mmrotate/models/losses/utils.py`.
# for Point2RBox-v3-jittor. SAM mask filtering logic (calculate_shape_metrics /
# filter_masks) used by VoronoiWatershedLoss's segment_anything branch.
#
# All computation is pure cv2/numpy and is kept line-for-line identical to the
# reference; only the framework-tensor handling changed: `original_image` (and
# `image`, which is only read for its H/W) may be a numpy array or a jt.Var
# (anything with .numpy()), where the reference accepted torch.Tensor.
#
# NOTE (verbatim-copy discipline, 铁律二): the built-in fallback filter_config
# below differs numerically from configs/point2rbox_v3/_base_sam-dotav1-0.py
# (e.g. default color_consistency weight 6 vs 2; class 11 carries
# rectangularity=-2; class 0/14 carry rectangularity=-1; different
# aspect_ratio_range). This mirrors the upstream file exactly — in training the
# config always passes mask_filter_config, so the fallback is only hit when
# filter_config=None. Do NOT "fix" either side.
...
def _as_numpy_image(x):
    """Accept np.ndarray or a framework tensor exposing .numpy() (jt.Var)."""
...
def calculate_shape_metrics(image, mask, required_metrics, original_image=None, aspect_ratio_range=None, prompt_point=None, debug=False):
    ...
    Args:
        ...
        original_image (np.ndarray or jt.Var, optional): Original image for color consistency calculation
    ...
    if 'color_consistency' in required_metrics and original_image is not None:
        if np.sum(mask) == 0:
            ...
            img_np, was_tensor = _as_numpy_image(original_image)
            if was_tensor:
